...
describe("shouldPersistQuery", () => {
  it("persists catalog and disk, not run snapshots", () => {
    expect(shouldPersistQuery(["episodes", "dropout", "clip", 0, 0])).toBe(true);
    expect(shouldPersistQuery(["disk", "dropout", "clip"])).toBe(true);
    expect(shouldPersistQuery(["series-list"])).toBe(true);
    expect(shouldPersistQuery(["series-missing", "dropout", "clip"])).toBe(true);
...
describe("onDiskSet", () => {
  it("maps slots to keys and prefers the disk payload", () => {
    ...
    const merged = mergeOnDisk([{ season: 4, episode: 11 }], [
      { on_disk: [{ season: 1, episode: 1 }] },
    ]);
    expect([...merged]).toEqual(["4-11"]);
    expect(mergeOnDisk([], [{ on_disk: [{ season: 1, episode: 1 }] }]).size).toBe(
      0,
    );
    const fallback = mergeOnDisk(undefined, [
      { on_disk: [{ season: 1, episode: 1 }] },
    ]);
